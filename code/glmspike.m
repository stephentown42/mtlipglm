...
classdef glmspike < neuroGLM
    ...
    methods
        ...
        function obj = glmspike(desc)
            ...
            obj.designOptions.MTsim             = false;        % Use MT simulation as input to LIP instead of stimulus
        ...
        % Args:
        %   basisName: string indicating name of setting, e.g...
                 % history, spike
                 % stim
                 % MTLIPcoupling
                 % MTcoupling
                 ...
                 % Targets
                 % Saccade
        %   duration: float, override on duration of basis functions
        %   nBasis: override on default number of basis functions
        %   nlScale: override on temporal offset                    (not sure why called nlScale)
        %
        % Returns:
        %   bs: Basis object containing smooth temporal basis functions
        ...
        %
        % Notes:
        %   trial: includes fields for every neuron, with naming convention that
        %   includes {region}neuron{id}
        %   gamma args are entered in msec
            ...
            if ~exist('gammaStim', 'var'), gammaStim = 500; end     
            if ~exist('gammaHist', 'var'), gammaHist = 250; end           
            if ~exist('gammaCoupleSame', 'var'), gammaCoupleSame = 250; end            
            if ~exist('gammaCoupleInter', 'var'), gammaCoupleInter = 500; end
            ...
            if gammaStim > 0 % if any stimulus terms are included
                ...
                if ~obj.designOptions.excludeMotion % are directional terms included
                    ...
                    elseif obj.designOptions.boxcarPulse
                        ...
                        motionOnsetBasis = obj.basisFactoryPresets('stim', [50 1100], 30, 500);     % smooth temporal basis: 'nonlinearly scaled cosine'
        ...
        %
            ...
            if ~exist('nFolds', 'var'), nFolds=5; end            
            if ~exist('rho', 'var'), rho=.2; end            
            if ~exist('colInds', 'var'), colInds = 1 : obj.edim; end
        ...
        %% predict spike rate
        function [predicted_rate, trialStarts, trialEnds] = predictSpikeRate(obj, M, trial, varargin)
        %[predicted_rate, trialStarts, trialEnds] = predictSpikeRate(M, trial, varargin)
        % 
        % Predict spike rate given stimulus covariates
        % 
        % Args:
        %   M: model fit
        ...
        %                                 as specified)
            ...
            for kCov = 1 : numel(Covariates)
                ...
                for kTrial = 1 : nTrials
                    ...
                    predicted_rate(iix) = predicted_rate(iix) + tmp;
                end
            end
            ...
            predicted_rate = M.fnlin(predicted_rate)*dt; % rate
            if any(isnan(predicted_rate))
                fprintf('rhat has nans\n')
            end
            if p.Results.spiking
                predicted_rate = poissrnd(predicted_rate);
            end
...
%             plotWeights(obj, M, plotSD)
            ...
            if ~exist('plotSD','var'), plotSD=false; end
            if ~isstruct(M)
            ...
            for k = 1 : n
                ...
                plot(wts.(f{k}).tr, wts.(f{k}).data, 'LineWidth', 1.5); hold on
                axis tight
                plot(xlim, [0 0], 'k')
                title(tstr{k})
            end
