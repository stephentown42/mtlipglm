classdef neuro < handle
    ...
    methods (Access=public)
        ...
        function dp = dPrimeSignedMotion(N)
            
            
    
            trialIx = intersect(N.nreader.trialIndex, find(N.stim.goodtrial));
   
            ev = [N.stim.timing.motionon] + [N.stim.timing.plxstart];
            ev = ev(trialIx);
    
            spcnt  = binSpTimes(N.nreader.spikeTimes, ev, [0.05 1.1], .05);
            r = sum(spcnt,2);
    
            pulses = reshape(N.stim.pulses(trialIx,:,:), numel(trialIx), []);
          
            dp = dprime(r, sum(pulses, 2)>0);
        end
...
%                 binEdges = [-5 -1 -.5 -.25 0 .25 .5 1 5];
        ...
        function S=whitenedPTA(N)
            % WHITENED PTA plots the whitened pulse-triggered average
            % N is an @neuro object
