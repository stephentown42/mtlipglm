classdef mtlipglm < handle
    ...
    methods
        %% Constructor
        function obj = mtlipglm(exname, varargin)
...
%             fprintf('exited loop at iter %d. %d events left\n', iter, numel(ev))
        ...
        function plotPTA(obj, neuronName, varargin)
            % plot Pulse-Triggered Average for neurons in neuronList
            ...
            neuronNames=findFile(fieldnames(obj.trial), 'neuron');
            assert(any(strcmp(neuronNames, neuronName)), 'Neuron names must be fields of trial struct')
            
            y=full(n.getBinnedSpikeTrain(obj.trial, neuronName, 1:numel(obj.trial)));
            
            sm=10; % 50ms smoothing
            smrate=smooth(full(y),sm)/.01;
            
%             dpcell=obj.neurons(kNeuron).dPrimeSigned;
            dpcell = 1;
            ...
            ptaBins = 100;
            % basic uncorrected
            [pta, ptaT]=computePTA(smrate, n, obj.trial, 3, ptaWin, ptaBins, dpcell);
            
            plot(ptaT, pta/nGabors)
            hold on
            plot(ptaT([1 end]), [0 0], 'k--')
            title(neuronName)
            xlim(ptaT([1 end]))
            xlabel('ms')
            ylabel('\Delta sp s^{-1}')
